...
After installing the skills, run `/clu-monitor` once in Claude Code to install a `SessionStart` hook. Every fresh session then arms a persistent Monitor on `clu watch --all --operator`, so wedges — stuck tools, blockers, refused gates, stalled claims — stream in live across every registered plan. The same hook also surfaces any **open blockers for the current project** — question, numbered options, and the `clu answer … --blocker` line — so a question raised before this session can be answered in it. Idempotent — re-running prints the current install status. Whether the hook is installed is read from `~/.claude/settings.json` itself, matched on the hook script's basename so a clu that moved is still recognised; the host database records only when it was installed. `clu install-hook --check` reports both without changing anything.
...
Where `clu watch` streams *events*, `clu top` is a `top`-like view of *processes* — one row per active worker showing what each is doing right now (phase progress, current command, last file write, last activity, heartbeat, PID liveness, last assistant line). A plan that's waiting on you surfaces as its own amber "blocked" row (the question + how long it's been waiting), and the header keeps running/blocked/dead tallies. The activity columns are read from each worker's own Claude Code transcript, so it's an independent check that workers are producing work rather than self-reporting it. It also surfaces **any live Claude session**, not just clu workers — your own interactive sessions show up as `sess` rows (named from the session's title), for registered projects plus any cwds you list in `session_dirs` (`~/.config/clu/config.json`). See `docs/operations.md` § "Watching workers — `clu top`".

<p align="center">
  <img src="docs/images/clu-top.png" alt="clu top — terminal dashboard of active workers" width="760">
</p>

`clu serve` puts that same `clu top` data — workers, blocked plans, and `SESS` rows alike — on a web page, with split / strip / phone layouts you cycle with `w` and a detail pane that tracks the cursor live. The detail view carries a scrolling activity feed — every assistant line, command, command output, file write, and `/code-review`-or-subagent spawn (an `agent` event), tailed incrementally from the selected session's transcript with sticky scroll — so fast-moving sessions are readable, not just glanceable. Localhost by default; `--lan` reaches it from your phone and flips on the full security layer at once (auto-generated token, Host-header allowlist for DNS-rebinding defense, auto self-signed HTTPS). Read-only, like everything else here. See `docs/operations.md` § "Serving the dashboard on the web — `clu serve`".

<p align="center">
  <img src="docs/images/clu-serve.png" alt="clu serve — read-only web dashboard of autonomous workers" width="760">
</p>

Don't have a live fleet to look at yet? `clu demo` stands up four synthetic `demo-*` workers — one busy, one idle, one blocked, one dead — driven through clu's real state pipeline, so the dashboards above light up with the exact rows you'd see in production. `clu demo --serve` also opens the web view in one shot. Ctrl-C (or `clu demo down`) tears the whole thing down; the demo fleet never notifies. It doubles as a verify-the-install smoke test.
...
- **`/clu-reply`** — explicit blocker reply for scripted or disambiguation contexts. Reply on your notify channel (the inbound poller routes it) or reach for `/clu-reply <plan-slug> <answer>` in-session. In-session reply is back without the inbox: the default `SessionStart` hook renders each open blocker's question and numbered options up front and tells the session to answer with `clu answer --project . --plan <slug> --blocker <q-N> <answer>`.
...
- **`/clu-monitor`** — one-shot setup skill that registers a `SessionStart` hook in `~/.claude/settings.json`. Every new session arms a persistent Monitor on clu's event stream and surfaces any open blockers for the project, so walking back to a session always has Claude already aware of what happened. Run once per machine; idempotent by deriving installed-ness from `settings.json` on every ask, so it can't nag you to install a hook you already have.
...
| Active-tool window (Claude Code) | `clu activity --plan S --phase X --token T --start-bash` (wired as `PreToolUse(Bash)`) and `--end-bash` (wired as `PostToolUse(Bash)`) — stamps `active_tool_started_at`, which scopes stuck-tool detection and holds the idle watchdog while a Bash call runs. The stamp is believed only while FRESH (bounded by `stuck_tool_threshold_seconds`): `PostToolUse` does not fire for a command that exits non-zero, so an unbounded stamp would let one failing test run silence the watchdog for the rest of the phase |
| Going quiet on purpose | `clu quiet-span --plan S --phase X --token T --reason code-review --expected-minutes 20` — a review or a full test gate looks exactly like a wedge from outside, so the worker says so and the idle watchdog holds its alert. A **lease, not a pair**: it expires on clu's clock (capped by `worker_quiet_span_ceiling_minutes`, default 45) whether or not `--end` is ever called, so a worker that dies mid-review can't leave the watchdog deaf |
...
| `clu watch [--plan SLUG\|--all] [--json] [--verbose] [--interval N] [--task-list]` | Stream state-machine events live — one line per transition. Default: every plan in the CWD project. SIGINT exits cleanly. Pair with Claude's `Monitor` tool for an in-session live feed. `--task-list` emits `TASK_CREATE`/`TASK_UPDATE` protocol lines for Claude's native TaskCreate UI (mutually exclusive with `--json`/`--all`) . On Opus 4.8 / Sonnet 5 / Fable 5 / Mythos 5 and newer, that UI needs `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` |
| `clu top [--project P] [--once] [--interval N] [--cols K,…]` | Read-only `top`-like dashboard of every active worker: phase, start elapsed, last command, last file write, last activity, heartbeat age, PID liveness, last assistant line. Plans waiting on you show as amber "blocked" rows (question + wait time); non-clu Claude sessions (your own interactive ones) show as `sess` rows for registered projects + any `session_dirs` cwds; the header tallies running/blocked/dead/sessions. Curses live view (`q` quits, `w` toggles detail); `--once` prints a plain snapshot. `--cols` picks a metric subset — beyond the 8 defaults, the modular extras `health` (one fused green/amber/red glyph for PID+ACT+heartbeat+stuck), `tokens`, `attempts` (X/max before halt), `lease` (countdown), and `progress` (phase X/N). Activity is read from each worker's Claude Code transcript — an independent check that workers are producing work, not just claiming to |
| `clu serve [--port P] [--project P] [--no-transcript] [--lan] [--host H] [--cert F --key F] [--http]` | Self-host the `clu top` dashboard as a read-only web page (`GET /` + a polled `GET /api/workers`) with split / strip / phone layouts (`w` cycles, `↑↓`/`jk` select). Same rows as `clu top` (workers, blocked, `SESS`); the detail pane streams the selected session's transcript feed — assistant lines, commands, writes, and `agent` events (`/code-review` + subagent spawns). Localhost-only by default; `--lan` binds one auto-detected LAN IP and flips on the full security layer at once — auto-generated token (`/login?token=` → `HttpOnly; SameSite=Strict; Secure` cookie, or `Bearer`), Host-header allowlist (`421` cross-origin, DNS-rebinding defense), and auto self-signed HTTPS. `--no-transcript` omits command/SAYING/write content; `--http` opts into LAN cleartext |
...
| `clu install-hook [--inbox] [--check]` / `clu uninstall-hook` | Register or remove the `SessionStart` hook in `~/.claude/settings.json` that arms the operator dashboard. `--inbox` additionally wires the retired `UserPromptSubmit` inbox surface. `--check` reports which clu hooks `settings.json` installs and when each was recorded, writing nothing. Entries are matched by the hook script's basename, so a reinstalled clu recognises its own hook instead of appending a duplicate. `/clu-monitor` is the user-facing wrapper |
