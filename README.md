...
Where `clu watch` streams *events*, `clu top` is a `top`-like view of *processes* — one row per active worker showing what each is doing right now (phase progress, current command, last file write, last activity, heartbeat, PID liveness, last assistant line). A plan that's waiting on you surfaces as its own amber "blocked" row (the question + how long it's been waiting), and the header keeps running/blocked/dead tallies. The activity columns are read from each worker's own Claude Code transcript, so it's an independent check that workers are producing work rather than self-reporting it. See `docs/operations.md` § "Watching workers — `clu top`".
