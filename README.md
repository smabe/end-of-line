...
| `clu top [--project P] [--once] [--interval N]` | Read-only `top`-like dashboard of every active worker: phase, start elapsed, last command, last file write, last activity, heartbeat age, PID liveness, last assistant line. Curses live view (`q` quits, `w` toggles detail); `--once` prints a plain snapshot. Activity is read from each worker's Claude Code transcript — an independent check that workers are producing work, not just claiming to |
