...
NEXT: p4 — read `plans/sqlite-migration-p4.md` FIRST.
Binding decisions for p4 (inline copy): token validation becomes an in-transaction compare-and-set (`WHERE plan_slug=? AND claimed_by=? AND phase_id=?`, rowcount 0 → `ClaimMismatch`) — the same predicate `assert_claim_match` evaluates today, now atomic, and the exception TYPE stays `st.ClaimMismatch` because the CLI decorator and the heartbeat daemon both catch it by name. Every native op bumps `plans.version` in its own transaction, since that is the per-plan change hint dashboards poll on and an op that forgets makes a live plan look idle in `clu top`. Events move to `events_archive` ONLY at explicit archival — a halted or paused plan keeps its events in the live table (operator, 2026-08-19), so `clu retry` and `clu resume` never pull rows back. Two notes from p3 that bind this phase: `plan_store.key_for_state_path` returns `(orch_dir, slug)` — the orchestrator dir, not the DB file, whatever the p4 sketch comment implies — and the `phase_started` event still carries NO `attempts` key, which this phase must not add.
