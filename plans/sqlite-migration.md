...
NEXT: p3 — read `plans/sqlite-migration-p3.md` FIRST.
Binding decisions for p3 (inline copy): `st.load(path)` / `st.mutate(path)` keep their signatures and the `Path` argument becomes a KEY — `<orch_dir>/<slug>.state.json` maps to (project DB at `<orch_dir>/clu.db`, plan `<slug>`) — while `config.state_path()` is untouched, so its slug validation and traversal guard still gate every external slug. **The load-bearing one: `Path.exists()` on a state path is a SECOND engine seam.** 21 sites gate on `state_path.exists()` before ever reaching `st.load`; leave one unconverted and it reports "no such plan" the moment state stops being written — the supervisor would idle every plan and the fleet would go dark with a green suite. Every one becomes `plan_store.exists`, and the phase carries `grep -rn "state_path.exists()\|state_file.exists()" end_of_line/` returning nothing as its own mechanical gate.
