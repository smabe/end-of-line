...
**Phase p2 — host DB: registry, monitor, iMessage/Discord sidecars, inbox, skill receipt** — SHIPPED `9e9e43f`
...
**Phase p3 — project DB: plan-state engine swap under the existing `st.mutate`/`st.load` API** — SHIPPED `fbcf31b`
...
**Phase p4 — native write ops for hot paths: heartbeat, activity stamps, worker callbacks, dispatch stamps** — SHIPPED `dcefd56`
...
**p2 SHIPPED 2026-08-19 (`9e9e43f`).** Every `~/.config/clu/*.json` store is now a table in the host database, behind unchanged public signatures.

**Spec check at p2** — three tasks, so run by an independent reviewer · tasks 3/3 evidenced · interfaces conform (Task 1's five preserved registry signatures, Task 2's `Produces: none`, Task 3's `claim_for_project`) · 7 uncovered, every one reported by the worker as required by the described work and recorded as a planning defect in p2's Work. Deviation from protocol, recorded because it is a real one: the reviewer's brief requires the diff PASTED into its prompt, and at 139KB that could not be transferred faithfully, so it was given the two exact commands that reproduce the diff deterministically and forbidden any other command or file access. The shard and the worker's report were pasted in full as usual.

**Review at p2** — `/code-review` at high: 3 findings, 2 fixed in the phase commit, 1 recorded as a mandated tradeoff. **`drain_outbound_marks` held the host write lock across a query of Apple's chat.db per pending mark** — harmless when the marks had their own file and their own flock, but they now share one database with the registry and the inbox, so a slow chat.db would make the supervisor silently DROP best-effort inbox notes rather than delay them; it also took `BEGIN IMMEDIATE` every 4s with nothing to drain. **`clu uninstall-hook` could crash with a traceback** after already rewriting settings.json, leaving a half-finished uninstall whose surviving marker still reported the hook as installed. Both fixes carry mutation-checked tests. **Not fixed: inbox delivery is now at-most-once** where it was at-least-once — upstream decision #3 requires the claim to be atomic with the read, which necessarily commits it before rendering finishes; the exposure was narrowed by moving the blocker walk ahead of the claim. Gate: **2216/2216 green** (2199 before the phase), basedpyright 0 errors, ruff clean.

Downstream sweep at p2 — p3 clean (its `Consumes` from p2 is only `registry.load_entry_state`, signature-preserved) · p4 clean · p5 clean (`registry.register` shipped signature-unchanged as its Consumes line declares) · p6 clean · p7 clean (`registry.entries` is one short SELECT, which is what its dashboard reads needed) · **p8 2 items corrected and 1 criterion added** — its quarantine list named neither `installed-skills.json` nor the `inbox/` directory, both of which p2 retired, so the quarantine would have left them behind and p8's own "no `*.json` store files outside `legacy/`" criterion would have failed on files nobody had scoped; and the four inert path-returning functions p2 left standing now have a deletion criterion there · code: p2 pinned every host store to one database and one lock, and the guards earlier phases shipped were unaffected — p1's `db.py` is the only earlier-phase source, it is additive, and nothing in it was built for the freedom p2 removed; the `except OSError` guards that DID become wrong were live production code rather than an earlier phase's, and all eight were widened in this phase · upstream: re-read `docs/design-briefs/sqlite-migration-scope.md`, unchanged since transcription; decision #3 (inbox reads + flags in one transaction) is implemented literally, and its at-most-once consequence is recorded in p2's shard rather than left implicit.

**The lesson p2 carries into every remaining phase:** consolidating N files into ONE database consolidates their LOCKS too. Anywhere that used to hold a narrow file lock across slow or foreign work now holds a host-wide or project-wide one. p3–p7 should look for that shape wherever a lock would span a subprocess, an HTTP call, or another database — it is what the `drain_outbound_marks` finding was, and p6's tick is the biggest instance of it.

**p3 SHIPPED 2026-08-19 (`fbcf31b`).** Plan state lives in the project database. Every consumer still calls `st.load` / `st.mutate`; the path became a key.

**Spec check at p3** — three tasks, so run by an independent reviewer · tasks 3/3 evidenced · **interfaces: the seven declared symbols all shipped with their exact signatures; TWO undeclared public symbols shipped alongside them** (`plan_store.exists_for_path`, `plan_store.write_full`) · 8 uncovered (one production file, `state_locator.py`, plus seven test files), every one reported by the worker as required by the described work. **Two defects in MY dispatch, recorded because they changed what the pass could see:** I told the reviewer to obtain the diff with `git diff HEAD`, which never shows untracked files — so `plan_store.py`, this phase's centerpiece, was invisible to it and it correctly refused to guess rather than inventing evidence. I closed Task 1's coverage and all seven signatures myself against the file. It also undercounted the `cli.py` gates at 13; there are 14, and all 21 sites across six files are converted.

**Review at p3** — `/code-review` at high: 2 findings. **Fixed: the zombie sweep's tolerant catch was complete for FILES and incomplete for a database.** Reading a state file could not be "busy" and its flock blocked rather than raising, so nothing in that catch list covered `db.DbBusy` — and with the write lock now covering the whole project, a tick working any other plan can hold it past the budget. The sweep died instead of skipping one plan; from `clu tick-all` that is caught per project, but `clu doctor` calls the sweep inline to print zombie health, so there it was a traceback where a health report belonged. Both windows now skip; two tests, both mutation-checked. **Recorded not fixed: the two undeclared symbols**, per the rule that approved interface lines are contract and are never edited to match what shipped — see p3's Decisions entry. Gate: **2251/2251 green** (2216 before the phase), basedpyright 0 errors, ruff clean.

**Verified independently rather than taken from the report:** after a full demo lifecycle the orchestrator directory holds only `clu.db` — no `*.state.json`, no `*.lock` — and `clu watch`'s output is byte-identical to the golden p1 captured from the JSON backend. The fleet-alive grep (`state_path.exists()` / `state_file.exists()`) returns nothing.

Downstream sweep at p3 — p4 clean plus the escalated criterion · p5 clean plus the criterion · p6 clean plus the criterion (its `Consumes` from p3 is `plan_store.snapshot`, shipped as declared) · **p7 1 item corrected plus the criterion** — it deletes `save_atomic` but nothing told it to delete `write_full`, which exists only to serve it and would have been left as exactly the orphaned-helper trap p2 deleted `registry_path()` to avoid; `exists_for_path` explicitly stays, since 21 live gates call it · p8 clean plus the criterion, and it inherits two more escape-hatch strings p3 surfaced: `cmd_init` still prints `Initialized <…>.state.json`, and `clu-phase/SKILL.md:22` still hands cold workers that path to read · code: p3 pinned plan state to one project-wide lock, and the guard built for the freedom it removed was the zombie sweep's own catch list, found and removed in this phase's review; p1's `db.py` and p2's host stores are untouched by the narrowing · upstream: re-read `docs/design-briefs/sqlite-migration-scope.md`, unchanged since transcription; decision #5's `clu state dump` ships here, and no transcribed decision conflicts with p3 as shipped or p4 as scoped.

**A finding CLASS has now recurred across two phases, so it is escalated to plan level.** p2 found eight `except OSError` guards a database failure would have walked straight through; p3 widened four more and still missed the zombie sweep's two, which its review caught. The shape never varies: a clause that was complete for a FILE is incomplete for a database, because a broken store arrives as `sqlite3.Error`, contention as `db.DbBusy` (a `RuntimeError`, so `except OSError` misses it), and a newer schema as `db.SchemaTooNew`. **Every unshipped shard — p4 through p8 — now carries it as a Done criterion**, phrased as the check rather than the intent: say which clauses were examined, which were widened, and which were deliberately left narrow.

**p4 SHIPPED 2026-08-19 (`dcefd56`).** The hot paths write single rows in their own transactions. The heartbeat no longer rewrites the event history.

**Spec check at p4** — two tasks, so run by an independent reviewer · tasks 2/2 evidenced (it reported Task 2's tests bullet as unbuilt; that bullet says re-point tests "where they asserted on write mechanics", and I verified none did — the existing callback families assert behaviour and pass unchanged) · **interfaces: 9 of 12 declared op signatures differ** — see p4's Decisions entry · 2 uncovered, both test files, both required by the described work. The reviewer was given the untracked file explicitly this time, after p3's audit lost a whole task to `git diff HEAD` not showing new files.

**Review at p4** — `/code-review` at high: 3 findings, 1 fixed, 2 recorded. **Fixed, and it is the one that matters: two assertions in this phase's own tests could not fail, including the one pinning its headline behaviour.** `st.utcnow()` is second-resolution — three consecutive calls return the identical string — so claiming a phase and beating within the same second makes "the heartbeat stamped this timestamp" true before the op runs. Proved by mutation rather than argued: breaking `op_heartbeat` to write the neighbouring column left all 53 tests in its own file green and the full 2307-test suite green. Both assertions now backdate first and check the neighbour is untouched; the same mutation now fails two tests. **Recorded not fixed:** the nine interface deviations, and the discovery that the worker-death dedup marker has no reader anywhere in the package — so a single worker death has always pinged the operator twice. Gate: **2307/2307 green** (2251 before the phase), basedpyright 0 errors, ruff clean.

Downstream sweep at p4 — **p5 1 criterion added** (timestamp-resolution) · **p6 2 criteria added** (timestamp-resolution; no git subprocess inside an open transaction) · **p7 2 criteria added** (the same two) · p8 clean · code: p4 pinned the hot write paths to row-level transactions and narrowed nothing an earlier phase had built a guard for — p3's `mutate_compat` is deliberately still carrying every caller p4 did not name, and p1's `db.py` is untouched. What it DID surface in earlier-phase source is the reverse shape: `_maybe_cleanup_worktree` still runs git subprocesses inside a `mutate` window from the archive path, which no phase owns and which p6 and p7 now carry as a criterion · upstream: re-read `docs/design-briefs/sqlite-migration-scope.md`, unchanged since transcription; decision #4's archive-table contract is implemented as the operator narrowed it at approval — a halted or paused plan keeps its events live, and only explicit archival moves them.

**The escalated `except`-clause class was discharged here and found its third instance in the process** — `dispatch._DISPATCH_FALLBACK_ERRORS` had been wrong for a database since p3, missing `sqlite3.Error`, `db.DbBusy` AND `st.LockTimeout`. Two more narrow guards in this phase's files (`cmd_unregister_one`'s terminalize, `cmd_worktree_gc`'s load) were widened rather than queued. The criterion stays on p5-p8.

NEXT: p5 — read `plans/sqlite-migration-p5.md` FIRST.
Binding decisions for p5 (inline copy): the queue and quota tables land, and the corruption-repair subsystem is DELETED rather than ported — transactions make the failure it repaired impossible, so `best_effort_extract_*`, `validate_repair`, the throttle functions, `cli._handle_corrupt_queue` and `dispatch.dispatch_repair_worker` all go. The pop becomes ONE project-DB transaction doing plan-create plus `queue.pop_head_if`, then a separate host-DB `registry.register` — the two-transaction shape upstream decision #2 specifies, with the head-check as a WHERE clause so a racing tick's loser is a no-op rather than a duplicate plan-create. Quota's "row absent == not paused" needs the singleton CHECK constraint p1 shipped. `clu quota clear` prints the cleared signature or "no quota pause recorded". A `.orchestrator.json` still carrying `repair_command` gets a one-line stderr deprecation note rather than silence. **This phase owns the plan-level zero-file criterion** — after it, a demo run leaves no `queue.json`, `quota.json`, `*.state.json` or `*.lock` anywhere.
