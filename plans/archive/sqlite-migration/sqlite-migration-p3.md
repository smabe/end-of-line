...
### Decision: `save_atomic` routes too, via a `write_full` the Produces line never declared  *(status: active — INTERFACE ADDITION, surfaced to the operator)*
- **What shipped:** `plan_store.write_full(orch_dir, slug, data)` — "replace this plan's rows wholesale", the store-side meaning of `save_atomic(state_path, data)` — plus `plan_store.exists_for_path(Path) -> bool`, the path-keyed form of the declared `exists`. Neither is on Task 1's approved `Produces:` line. All seven symbols that ARE on it shipped with their exact declared signatures.
- **Rationale:** the Work list said `load` and `mutate` route. It did not anticipate that 34 test files seed plan state through `save_atomic` at 68 sites, usually inside `with st.locked(sp):`. Routing the third primitive kept all of them working; the alternative was rewriting 34 files by hand. No production writer reaches `write_full` after the engine swap — only `queue.json` and `quota.json` still touch the file engine — so its blast radius is tests only.
- **Why this is recorded rather than absorbed:** approved interface lines are contract, and the fix for a divergence is never to edit the plan to match what shipped. `exists_for_path` is mechanical and sits inside this phase's own locked decision that the path stays the API key. `write_full` is a real design addition the operator never saw, and it is theirs to overturn.
- **Disposition:** p7 now carries its deletion, in the same breath as `save_atomic` — it exists only to serve a primitive that phase removes.

### Decision: `BLOCKING_TIMEOUT_S = 60.0` replaces "block forever"  *(status: active — a NEW failure mode)*
- **Rationale:** `st.mutate(timeout_seconds=None)` waited on the flock indefinitely. `db.connect`'s 5s default would have turned that into a `DbBusy` fast enough to break a heartbeat landing during a tick's reap, so an explicit budget was needed. 60s is longer than every real contender — the tick's window spans a reap poll (up to 5s), a coolant subprocess and `ps` calls — while still being bounded, so a genuinely wedged holder becomes audible instead of hanging the caller forever.
- **What is new:** an unbounded wait cannot time out; this one can. A caller that would have waited now raises `LockTimeout` after a minute.
- **Alternatives considered:** keep it unbounded (a wedged holder silently freezes every writer in the project, which is worse under a project-wide lock than it was under a per-plan one); use `db.connect`'s 5s default (demonstrably too short for the tick window).

### Finding: the project-wide lock is now held across subprocesses  *(empirical, this phase — p6 is where it is removed)*
p2's lesson landed exactly where p2 predicted it would. `supervisor.tick`'s `mutate` window spans `reap_claim` (which polls up to 5s), coolant's `emit_stop`, and `ps` calls. Under the flock that blocked ONE plan's file; under the store it blocks every plan in the project. `sweep_zombie_states` has the same shape. This phase mandates that window — the whole point of `mutate_compat` is one transaction around the read-modify-write — so it is not a defect here, but it IS the concrete thing p6's tick restructure exists to remove, and it is the reason the timeout above had to be generous.

### Finding: `exists` cannot raise, so what each failure ANSWERS is load-bearing  *(empirical, this phase)*
It replaces a filesystem call that had no failure mode, and the 21 gates that ask it have no except clause. Missing DB, newer schema, or momentarily busy → **no**, exactly as a missing state file answered. A DB present but unreadable → **yes**, because a corrupt state file also passed the old gate and then failed the load, which is what put the plan's name in front of the operator. Answering "no" there made a corrupt project look like an empty one and silently dropped `clu tick-all`'s per-plan error report — caught by `test_tick_all`, not by reasoning.

### Finding: `key_for_state_path` returns `(orch_dir, slug)`, not `(db_path, slug)`  *(empirical, this phase — binds p4)*
The sketch's comment said the DB file. Every other function in the same sketch takes `orch_dir`, and p4 consumes this function to feed exactly those, so the orchestrator dir is what it returns. The declared type `tuple[Path, str]` is unchanged, so this is a comment that was wrong rather than an interface that moved. **p4, p5 and p6 should read it as the orchestrator dir.**

### Finding: event dicts now carry `id`, and one JSON surface shows it  *(empirical, this phase)*
`clu status --json` and `clu watch --json` gain one field per event. Mandated by this phase's own locked decision that events carry the table's autoincrement id, and additive rather than a format change. The TEXT projection the golden pins is unaffected — verified byte-identical against p1's pre-migration capture.

### Finding: three plan-text defects this phase exposed  *(empirical, this phase)*
The shard says "the three flock-mechanism tests" — the master's own Background findings correct that to TWO (`test_state.py:552-573` and `test_activity_callback.py:166-186`), with two others miscounted as lock tests when they actually test atomic rename. Converting two was right. The shard also lists 14 `cli.py` exists-gates and 21 total; all 21 are converted and the mechanical grep returns nothing. And `cmd_init` still prints `Initialized <…>/<slug>.state.json`, naming a file that is now only a key — left alone as a user-visible surface this phase does not name, and carried to p8's "no escape hatch names a file that isn't there" sweep along with `clu-phase/SKILL.md:22`, which still hands cold workers that path to read.

### Finding: eight files the approved Work list did not name  *(empirical, this phase)*
One production file — `end_of_line/state_locator.py`, one line plus a docstring: a `json.JSONDecodeError` catch that had to widen, or a single corrupt project database crashes the inbound reply router's whole fleet walk instead of skipping one plan. Seven test files beyond the ones named. **What the plan should have seen:** it enumerated the raw READERS of state files but never asked which tolerant `except` clauses name exception types the engine change would invalidate — the same mechanical grep p2's sweep already showed was missing. That is the class now escalated to every unshipped shard.
