...
| `monitor.py` | Whether each clu hook is installed, derived per surface from `~/.claude/settings.json` in three states; plus the account-wide install record in the host database's `monitor` table. Used by the `/clu-monitor` skill, the CLI tip-suppression branch, and the install/uninstall paths. |
