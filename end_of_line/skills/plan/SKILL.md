---
name: plan
description: Create, resume, hand off, or ship a phased plan (single file for single-phase work; master + one shard per phase for multi-phase) in plans/ before starting multi-file work. Keeps ADHD scope creep in check by making scope explicit and approved upfront.
user_invocable: true
---
...
4. **EXPLORE — mandatory, no skip conditions, no flag opt-out.** This is the E in EPCC (Explore → Plan → Code → Commit). The plan is *not* drafted until exploration completes. There is no "small plan" exception, no "I already read the files" escape, no `--no-research` opt-out, no "pure docs/config" carve-out. Codebase + project-local API docs + web prior art — all three dimensions, every time, before a single line of plan text gets written.
   ...
   **Consolidate findings as ground truth for the plan** (internal step; not surfaced to the user as a separate report). Walk away from exploration with:
   ...
   - **No unverified claims survive EXPLORE.** There is no `TODO: verify` channel. If research couldn't close a question, that's the signal EXPLORE isn't done — finish it before drafting. The only things research legitimately can't close are (a) genuine operator decisions (surfaced at approval) and (b) empirical/runtime unknowns (which become the Diagnosis falsifiable test or the algorithmic load-test, never plan-body facts). For anything else that can't be verified without external access or a running app, STOP before drafting and resolve it with the operator — never carry an unverified claim into the draft.
...
5. **Draft the plan** using the template below, with research findings as ground truth. Three rules for drafting:
   ...
   - **Verify or block — no deferral channel.** Every cited file path, API name, metadata key, version number, framework behavior, or external system claim must be verified during exploration and stated as fact with a file:line or URL+section citation. If you can't verify it, you don't draft the plan: STOP and resolve it with the operator first (provide access, run the thing, or pull it out of scope). There is no `TODO: verify`, no "I'll confirm during implementation," no placeholder. The line between "extrapolation from training data" and "verified this session" is enforced by absence — every claim in the plan is verified, or the plan isn't written yet.
...
6. **Write the file(s).** Single-phase plan → one file `plans/<slug>.md`. Multi-phase plan → the master `plans/<slug>.md` AND every shard `plans/<slug>-<phase>.md`, written in the same step. **Drafting the shards is not optional and not deferrable to "when the phase starts" — a multi-phase plan whose shards don't exist yet is not written.** Every shard a phase needs to be self-sufficient is authored now, at plan time, from the same research pass; a future session resuming the plan reads a shard that exists or it has nothing to resume from.
...
4. **Ask what the user wants**: continue working it, **re-plan it** (if step 2 flagged staleness), update the plan, or ship it (archive to `plans/shipped/`).
...
3. **Move the master AND every shard.** `plans/<slug>.md` → `plans/shipped/<slug>.md`, and each `plans/<slug>-<phase>.md` → `plans/shipped/<slug>-<phase>.md`, using `git mv` for tracked files, plain `mv` otherwise. **Never archive the master and leave shards orphaned in `plans/`** — they move together as one unit; the shards carry the decisions-and-findings record that makes the archive worth keeping.
4. **Confirm to the user** with the new paths and a one-line summary of what shipped.
...
Two shapes, chosen by a **mechanical rule — NOT a judgment call.** Count the phases: **exactly one phase → the single-file plan. More than one phase → master + one shard per phase.** There is no third option and no "small multi-phase" middle ground — two phases shard, six phases shard, the rule is the same. The Scope Check, Phase Completion Cycle, and Rules below refer to these section headers by name — don't rename them.
...
- **Decisions & findings (in the shard)**: Each phase's research and decisions live INSIDE that phase's shard — NEVER in a separate research file, NEVER only in the master. One entry per **non-local** decision (consequences scattered across the code, not contained in one spot); local one-liners stay in the master's Non-goals. Each entry is Decision / Rationale / Alternatives considered / Evidence (file:line or URL+section). Mark a decision a later phase invalidates as `superseded by phase-<id>` — don't silently rewrite it. Append empirical findings (spike results, mid-implementation gotchas) here as the phase runs; this is what stops the next session rediscovering them after a clear.
...
- **Failure modes**: Aim for 5+. If you have fewer than 3, you don't understand the problem yet. Draw from: similar past failures, platform quirks, unfamiliar dependencies, integration boundaries, untested paths. **But this section is not a sink for unresolved verifications.** An entry must be a genuine runtime/integration risk — not a fact you didn't look up, wearing risk's clothing. A conditional whose antecedent is statically checkable ("*if* warmups can't be marked complete, the predicate drops them"; "*if* this endpoint doesn't return X…") is exactly that: resolve the antecedent during EXPLORE (Read/grep/doc-fetch), then either delete the entry or restate it as the verified fact. Phrasing an unverified fact as an "if" does NOT exempt it from verify-or-block — the rule bites on the antecedent, not just on declarative claims. A legitimate failure mode is one whose outcome remains uncertain *after* you've verified everything statically knowable about it.
...
2. **Simplify** — run your project's review pass (`/code-review` or an equivalent) on the changed code. **Trivial-diff escape hatch**: skip it only if the diff is single-file AND single-logical-change AND has no behavior change (typo fix, version bump, comment rewording, doc-only edit). When in doubt, run it.
...
- **Multi-phase plans MUST shard — this is mechanical, not a judgment call.** More than one phase → a master `plans/<slug>.md` PLUS one shard `plans/<slug>-<phase>.md` per phase. The single-phase plan is the ONLY non-sharded shape. There is NO "small multi-phase" exception, NO "I'll keep these two phases in one file," NO "it reads cleaner as one document," NO "fewer files is simpler." Two phases shard; six phases shard; the threshold is exactly the >1-phase line that triggers the Phase map and Status & cold-start. Writing a multi-phase plan as a single file — or letting per-phase detail pool into one growing document — is the failure this rule exists to stop. The instant you notice you're about to put a second phase's work in the first phase's file, STOP and shard. "Fewer files" is never the reason to collapse; simplicity is conceptual load (one phase = one self-contained shard), never file count.
...
- **The template is the source of truth** — there are two shapes (single-file plan; multi-phase master + per-phase shard), and the Scope Check, Phase Completion Cycle, Mode 2/3/4, and Rules all refer to their section names. Don't add, remove, or rename a section in either shape without updating every one of those references.
...
- **A resumed plan is a hypothesis, not a license.** Before working any pre-existing plan (Mode 2), validate the NEXT shard's Work (vs. the master's Files touched overview), the Diagnosis, and the approach against the current code **in this session** (Mode 2 step 2). If scope or approach has materially diverged, re-run Mode 1 step 4 EXPLORE and rewrite — never patch a stale plan turn-by-turn while coding. Switching to a smaller-diff approach than the plan names is itself a re-plan trigger; "smaller diff / fewer files / lower risk because less code" is never the reason to switch off the structural approach — only proven infeasibility (cited) is.
...
- **Perf/bug plans: run the Diagnosis falsifiable test BEFORE scoping any shard's Work.** A plan whose goal is to change something that's already running (perf regression, bug fix, "stop X from doing Y", "make Z cheaper") needs ground truth on what's actually causing the symptom before we scope a fix. The Diagnosis section's hypothesis + falsifiable test exists for this. If the test confirms the hypothesis, scope normally. If it disproves the hypothesis, the rest of the plan is built on sand — don't write the file list, return to Mode 1 step 4 (Explore) with the negative result as the sharper question. Files-read alone doesn't ground the diagnosis; "I commented out X and the symptom didn't change" does. Two failed disable-experiments back-to-back means the hypothesis space is wrong — switch to a dedicated diagnosis pass, don't keep guessing.
