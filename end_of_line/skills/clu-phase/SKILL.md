...
4. `<state_file>` — the plan's state KEY (e.g. `/Users/me/projects/foo/plans/.orchestrator/multi-plan-routing.state.json`). **It is path-shaped, and there is no file behind it.** Plan state lives in the project database `plans/.orchestrator/clu.db`; this argument names (that database's directory, that plan). Two things follow, and both matter:
   - **Never `cat`, `jq`, `grep` or edit it.** To read prior history, ask clu: `clu state dump --project "$PROJECT_ROOT" --plan "$PLAN"`.
   - **Its DIRECTORY is still exactly right**, which is why the steps below derive `$PROJECT_ROOT` and the log/context paths from `dirname "$STATE"`. That is not a leftover — it is the supported way to find the orchestrator dir from what you were handed.
...
| Run `clu complete / block / heartbeat-daemon / prior-blocker` — pass `--project ...` | **`$PROJECT_ROOT`** (canonical). clu's database + registry live here, not in the worktree. |
| Inspect the plan's state or event history | `clu state dump --project "$PROJECT_ROOT" --plan "$PLAN"` — run it BARE, like every other `clu` call. Never open `$STATE`; there is no file there. |
| Derive `$PROJECT_ROOT`, the worker log dir, or the attempt-context path | `dirname "$STATE"` — the key's directory IS the orchestrator dir. No `cd` needed. |
...
2. **Arm the heartbeat daemon.** Long phases that don't heartbeat look identical to hung workers — `clu status` reports `STALLED` and the supervisor's gap-fill notifications fire. One flat command arms it:
   ...
   ```
   The command validates the token against the live claim (stamping the first heartbeat in the same stroke — a forged or stale token dies loudly here), then double-forks into a detached daemon that pings every 2 minutes while the worker PID is alive. The PID comes from the claim — the dispatcher stamps your Claude Code process's PID there at spawn, so you don't pass it (`--worker-pid` exists as an explicit override only; `$PPID` is never available under scoped-permission dispatch, and in headless `--print` sessions shell variables don't persist across Bash calls — any `$VAR` an example here references must be defined in the same Bash call that uses it, or replaced with the literal value). It shuts itself down: when the worker PID dies (SIGKILL/OOM included, issue #72), or on the first token rejection after the claim is released — your `clu complete`/`clu block` releases the claim, so the daemon is gone within one 120s tick of your exit. No background bash loop, no cleanup step: the old `( while ...; do clu heartbeat; ...; done ) &` compound is denied under scoped-permission dispatch (subshell/loop constructs don't survive permission decomposition — #90 spike Test B), which is why this is a single allowlistable command.
   ...
   The 2-minute interval is well inside the heartbeat threshold (derived from lease TTL: `min(25, max(15, lease_ttl//2))`, so 25 min at the default 60-min lease) and loose enough not to flood the store with writes. The watchdog layers and their semantics are unchanged from the bash-loop era: (1) the daemon's worker-PID liveness probe catches death-by-signal; (2) the token-rejection exit catches graceful completion; (3) supervisor-side `_detect_dead_pid` catches the worst case where the daemon itself dies; (4) the 3-strike self-report texts the operator if heartbeats silently stop landing; (5) `_emit_worker_idle` (supervisor-side, wedge-watchdogs phase 2) catches the orthogonal "wedged mid-API-stream" case: PID alive, no Bash tool active, CPU ≤1% over ≥10min, no open Anthropic socket.

2b. **Activity-hook environment — dispatcher-provided, nothing to do.** The dispatcher injects `CLU_PLAN` / `CLU_PHASE` / `CLU_TOKEN` / `CLU_PROJECT` into your process env at spawn; the Claude Code PreToolUse/PostToolUse activity hook (operator-installed — see "Activity hook (operator setup)" at the bottom of this SKILL) inherits them automatically and stamps the Bash-call window the stuck-tool detector scopes to. Workers without the hook installed produce zero `tool_stuck` events; lease expiry is the safety net.

3. **Check for resume**: run `clu prior-blocker --project "$PROJECT_ROOT" --plan "$PLAN" --phase "$PHASE"` (see "Resume-after-answer" above). If it prints an answer, the user replied to a question you asked on a previous attempt — factor that answer into your plan for this run. For the full picture, `clu state dump --project "$PROJECT_ROOT" --plan "$PLAN"` prints the plan's whole state document, event log included.

4. **Read the master plan** at `$WORKTREE_ROOT/plans/$PLAN.md` (the plan files live on your branch in the worktree — same content as `$PROJECT_ROOT/plans/`, but reading from your worktree keeps you anchored). Find the row in `## Sessions index` whose phase id matches `$PHASE` (the parser strips the master-stem prefix from the plan_file basename — see `end_of_line/plan_parser.py`). The row's "Plan file" cell points to the sub-plan markdown. **Also read the master's `## Findings log`** if present — prior phases append runtime findings there (gotchas, spike results, surprises, assumptions that proved wrong) that bear on your work. It's empty, or absent, if you're the first phase.
...
7. **Do the work.** Use the editing/test/commit tools you have. Follow the project's CLAUDE.md (TDD, /code-review after non-trivial work, structured commit messages, etc.). **If this phase surfaces something a later phase would otherwise rediscover** — a gotcha, a spike result, an API surprise, an assumption that proved wrong — append one dated bullet to `## Findings log` in `$WORKTREE_ROOT/plans/$PLAN.md` (create the heading if the master predates the convention). When you know the finding before committing, stage it with the phase commit so it rides the SHA your attestations stamp; a finding that only surfaces after the commit (e.g. from the post-commit verify run) rides a small follow-up commit — re-stamp verify/simplify against the new HEAD per the pre-complete rules. Either way the next phase reads it from the master at its step 4. Skip when nothing surfaced. When you commit, capture the SHA — `git rev-parse HEAD` after each `git commit`. **Before every `git commit`, verify the branch:** `git rev-parse --abbrev-ref HEAD` should print `clu/$PLAN` (NOT `main`). If it prints `main`, you've drifted out of the worktree — `cd "$WORKTREE_ROOT"` and re-stage before committing.
...
- **Running a phase outside clu dispatch**: the dispatcher injects `CLU_PLAN` / `CLU_PHASE` / `CLU_TOKEN` / `CLU_PROJECT` into the worker process env at spawn (step 2b). If `/clu-phase` is invoked by hand — no dispatcher — those vars are unset, the Claude Code PreToolUse hook (if installed) short-circuits, and `tool_stuck` detection is silently disabled. That short-circuit is by design; the phase still works, you just lose the early-warning signal for wedged Bash subprocesses. Don't try to compensate with worker-side `export` — env doesn't persist across Bash tool calls in headless `--print` sessions, which is exactly why injection moved to the dispatcher.
